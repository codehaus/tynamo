...
/**
 * Manages the JPA transaction for the current thread. This includes creating an entity manager as
 * needed, allowing the em to checkpoint (commit the current transaction and continue) and commit
 * the transaction automatically at the end of the request.
 * <p/>
...
public interface JPATransactionManager
{
	 ...
	 * 
	 * @return the request's entityManager
	 * @see JPAEntityManagerSource
	 */
	EntityManager getEntityManager();

	/**
	 * Commits the current transaction (which will cause a flush of data to the database), then
	 * starts a new transaction to replace it.
	 */
	void commit();
